...
function calculateBoxSize(numMolecules: number, density: number): number {
  ...
  const boxSize = volumeAngstroms ** (1 / 3);
...
function generateWaterBox(numMolecules: number, boxSize: number): Frame {
  ...
  for (let i = 0; i < numMolecules; i++) {
    ...
    const water = generateWaterMolecule(
      centerX,
      centerY,
      centerZ,
      rotX,
...
async function main() {
  ...
  console.log(
    `  - Number of water molecules: ${NUM_WATER_MOLECULES.toLocaleString()}`,
  );
  console.log(`  - Density: ${DENSITY} g/cm³`);
  console.log(
    `  - Box size: ${boxSize.toFixed(2)} Å (${(boxSize / 10).toFixed(2)} nm)`,
  );
  ...
  console.log(
    `✅ Molvis initialized in ${(endInit - startInit).toFixed(2)} ms`,
  );
  console.log();
