...
class EditMode extends Mode {
        ...
        if (pointerInfo.pickInfo) {
            ...
            if (picked_mesh) {
                ...
                if (!this._is_dragging && this._pos_on_mouse_down) {
                    ...
                    this._app.add_atom(System.random_atom_id(), xyz.x, xyz.y, xyz.z, new Map());
                }
                // else: drag empty
            }
        }
    ...
    _on_mouse_move = (pointerInfo: PointerInfo) => {
        ...
        if (this._mesh_on_mouse_down && this._is_dragging) {
            ...
            // hit selected mesh
            if (this.is_first_move && this._mesh_on_mouse_down.name.startsWith("atom")) {

                const atom_mesh_on_dragging = this._mesh_on_mouse_down.clone(`_dragging_atom`, null)!;
                ...
                pointer_drag_bahavior.onDragObservable.add((event) => {
                    bond_mesh_on_dragging = bond_mesh_on_dragging;
