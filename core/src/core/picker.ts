...
import { HitResult } from "../mode/types";
...
export class Picker {
    ...
    public async pick(x: number, y: number): Promise<HitResult> {
        ...
        return {
            type: meta.type as any, // "atom" | "bond"
            mesh,
    ...
    private ensurePickingMaterials() {
        if (!this.atomPickingMaterial) {
            this.atomPickingMaterial = new ShaderMaterial(
                ...
                {
                    attributes: ["position", "uv", "instanceData", "instanceColor", "instancePickingColor"],
            ...
            this.atomPickingMaterial.onBindObservable.add((_mesh) => {
                ...
                this.atomPickingMaterial!.setMatrix("projection", this.scene.getProjectionMatrix());
        ...
        if (!this.bondPickingMaterial) {
            this.bondPickingMaterial = new ShaderMaterial(
                ...
                {
                    attributes: ["position", "uv", "instanceData0", "instanceData1", "instanceColor0", "instanceColor1", "instanceSplit", "instancePickingColor"],
                    uniforms: ["view", "projection", "uPickingEnabled"]
                }
            );
            ...
            this.bondPickingMaterial.onBindObservable.add((_mesh) => {
                ...
                this.bondPickingMaterial!.setMatrix("projection", this.scene.getProjectionMatrix());
    ...
    private renderPickingScene() {
        ...
        this.scene.meshes.forEach(m => {
            const isAtom = m.name.startsWith("atom_base") || m.name.startsWith("edit_atom_base") || m.name.startsWith("manip_atom_base");
            const isBond = m.name.startsWith("bond_base") || m.name.startsWith("edit_bond_base") || m.name.startsWith("manip_bond_base");

            if (isAtom && this.atomPickingMaterial) {
                this.pickingTexture!.renderList!.push(m);
                this.pickingTexture!.setMaterialForRendering(m, this.atomPickingMaterial);
            } else if (isBond && this.bondPickingMaterial) {
                this.pickingTexture!.renderList!.push(m);
                this.pickingTexture!.setMaterialForRendering(m, this.bondPickingMaterial);
            }
        });
 ...
 */
export function encodePickingColor(meshId: number, thinId: number): [number, number, number, number] {
    const fullId = ((meshId & MAX_MESH_ID) << INSTANCE_ID_BITS) | (thinId & MAX_INSTANCE_ID);
