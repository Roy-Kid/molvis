...
class AxisViewer {
    ...
    private createAxis(
        ...
        // Shaft
        const shaft = MeshBuilder.CreateCylinder("shaft" + label, {
        ...
        // Arrow Cap (Cone)
        const arrow = MeshBuilder.CreateCylinder("arrow" + label, {
            height: arrowHeight,
            diameterTop: 0,
        ...
        const finalPos = Vector3.TransformCoordinates(directionVec.scale(labelOffset), rotationMatrix);
    ...
    private createLabel(text: string, color: string) {
        ...
        const dynamicTexture = new DynamicTexture("DynamicTexture", 128, this._scene, true);
