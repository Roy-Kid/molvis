...
import { Box, Frame } from "molrs-wasm";
...
export class DrawBoxCommand extends Command<void> {
    ...
    do(): void {
        ...
        this.app.world.sceneIndex.registerBox({
            mesh: this.boxMesh,
            meta: {
                dimensions: (() => { const l = this.box.lengths().to_js_array(); return [l[0], l[1], l[2]] as [number, number, number]; })(),
                origin: (() => { const o = this.box.origin().to_js_array(); return [o[0], o[1], o[2]] as [number, number, number]; })()
            }
        });
    }

    private createEdges(corners: Float32Array, radius: number, material: BABYLON.Material, scene: BABYLON.Scene) {
        ...
        const getPoint = (idx: number) => new Vector3(corners[idx * 3], corners[idx * 3 + 1], corners[idx * 3 + 2]);

        for (const [i, j] of edges) {
            const p1 = getPoint(i);
            const p2 = getPoint(j);

            const tube = BABYLON.MeshBuilder.CreateTube("box_edge", {
                path: [p1, p2],
                radius: radius,
                tessellation: 8,
...
export class DrawAtomCommand extends Command<{ atomId: number }> {
    ...
    constructor(
        ...
        super(app);
        this.atomId = options.atomId ?? ((app.world.sceneIndex.getNextAtomId && app.world.sceneIndex.getNextAtomId()) || 0);
    }

    do(): { atomId: number } {
        const result = this.app.artist.drawAtom(this.position, { ...this.options, atomId: this.atomId });
...
export class DeleteAtomCommand extends Command<void> {
    private savedAtomData: any | null = null;
    ...
    private deletedBonds: Array<{ bondId: number; data: any }> = [];
...
export class DrawBondCommand extends Command<{ bondId: number }> {
    ...
    constructor(
        ...
        super(app);
        this.bondId = options.bondId ?? ((app.world.sceneIndex.getNextBondId && app.world.sceneIndex.getNextBondId()) || 0);
    }

    do(): { bondId: number } {
        const result = this.app.artist.drawBond(this.startPos, this.endPos, { ...this.options, bondId: this.bondId });
