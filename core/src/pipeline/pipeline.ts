...
export class ModifierPipeline extends EventEmitter {
    ...
    addModifier(modifier: Modifier): void {
        ...
        this.emit(PipelineEvents.MODIFIER_ADDED, { modifier, index: this.modifiers.length - 1 });
    }
    ...
    reorderModifier(modifierId: string, newIndex: number): boolean {
        ...
        this.emit(PipelineEvents.MODIFIER_REORDERED, { modifier, oldIndex, newIndex });
        return true;
    }
     ...
     */
    async compute(source: FrameSource, frameIndex: number = 0): Promise<Frame> {
        // Load initial frame
        let frame = await source.getFrame(frameIndex);

        // Create initial context
        let context = createDefaultContext(frame, frameIndex);
