...
- `stage/` — public React-free 3D Babylon engine peer; depends inward on core and neither imports sketch nor accesses molrs directly. It owns the command/pipeline/rendering stack: `DataSourceModifier` remains the single scene-data ingress, and `classifyFrameTransition` threads `changeKind: "position" | "full"` into `PipelineContext` to pick buffer-update vs full rebuild.
