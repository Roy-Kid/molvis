...
## Color by source (multi-source overlay)

Coloring overlaid structures by their originating source is **not a new modifier** — it is a `categorical` mode on the existing `ColorByPropertyModifier` (`modifiers/ColorByPropertyModifier.ts`). The modifier already routes String columns through the categorical palette (`buildCategoricalColorLookup`, glasbey-vivid) and numeric columns through the viridis ramp; the `categorical` flag makes a *numeric* column (the Int32 `source_id` emitted by `CombineSystemsModifier`) take the categorical path instead — each distinct id (natural-sorted ordinal) maps to `palette[i % N]`, wrapping past the palette length. Render-state ownership is satisfied without touching `StyleManager`: the color is the modifier's serializable config (`columnName="source_id"` + `categorical=true`), re-injected as `__color_r/g/b` on every `DrawFrameCommand` pipeline run; the GPU buffer is never the source of truth. `isApplicable` gates the picker so the mode is hidden when no `source_id` column is present. `buildSourceColorLegend(sourceIds)` (`artist/palette.ts`) exposes the same ordinal→hex mapping for a UI legend.
