...
- **UI** lives in the View tab's render area (`RenderTab.tsx`): turntable speed control (revolutions / total duration), a play/stop preview toggle, and an export button. The export button calls `app.exportTurntable`, receives `dataURL[]`, and a page-local `gif-encode.ts` encodes GIF/WebM (gif.js or `MediaRecorder`) and triggers download. Encoding stays in `page/` — core ships only `dataURL[]` and pulls in no encoding deps.
...
- `page/src/ui/modes/view/RenderTab.tsx` — turntable controls + export button
- `page/src/ui/modes/view/gif-encode.ts` (new) — `dataURL[]` -> GIF/WebM encode + download
