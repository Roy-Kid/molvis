...
- [worker-arch-unify-01-channel](worker-arch-unify-01-channel.md) — core workload 信道扩展：host-call 反向请求、interleaved 调度、reject-cancel、ready 超时 [approved]
...
Chain `worker-arch-unify`: 01 → 02 → 03 → 04（严格串行；03 为 stage+vsc-ext 最小原子 seam，04 依赖 alias 已就位）。与 `traj-ingest` 的交点：`TrajectorySource.readRange` seam 形状冻结（04-range/05-remote 依赖）。
