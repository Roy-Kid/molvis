...
import { Frame, ArrayFrameSource, readFrame } from '@molvis/core';
...
export const DataSourceModifier: React.FC<DataSourceModifierProps> = ({ modifier, app, onUpdate }) => {
  ...
  const handleClear = () => {
       ...
       if (app && (app as any).commandManager) (app as any).commandManager.clearHistory();
  ...
  return (
    <div className="space-y-4">
        <div className="flex gap-2">
            ...
            <Button variant="ghost" size="icon" onClick={handleClear} title="Clear Scene" className="h-9 w-9 text-destructive hover:text-destructive hover:bg-destructive/10">
                ...
                <tbody className="divide-y">
                    <tr className="hover:bg-muted/50 transition-colors">
                        <td className="px-3 py-2">
                            <Checkbox 
                                checked={showAtoms} 
                                onCheckedChange={(c) => handleToggle('showAtoms', c as boolean)} 
                                className="h-3.5 w-3.5"
                            />
                        </td>
                        <td className="px-2 py-2 font-medium">Atoms</td>
                        <td className="px-3 py-2 text-right font-mono text-muted-foreground">{atomCount}</td>
                    </tr>
                    <tr className="hover:bg-muted/50 transition-colors">
                        <td className="px-3 py-2">
                            <Checkbox 
                                checked={showBonds} 
                                onCheckedChange={(c) => handleToggle('showBonds', c as boolean)} 
                                className="h-3.5 w-3.5"
                            />
                        </td>
                        <td className="px-2 py-2 font-medium">Bonds</td>
                        <td className="px-3 py-2 text-right font-mono text-muted-foreground">{bondCount}</td>
                    </tr>
                    <tr className="hover:bg-muted/50 transition-colors">
                        <td className="px-3 py-2">
                            <Checkbox 
                        ...
                        <td className="px-3 py-2 text-right font-mono text-muted-foreground">{hasBox ? 1 : 0}</td>
                    </tr>
                </tbody>
            </table>
        </div>
