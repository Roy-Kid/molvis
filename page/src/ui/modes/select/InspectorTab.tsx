...
import { Molvis, type SelectionState, parseSelectionKey } from '@molvis/core';
...
export const InspectorTab: React.FC<InspectorTabProps> = ({ app }) => {
    const [selection, setSelection] = useState<SelectionState>({ atoms: new Set(), bonds: new Set() });
    ...
    useEffect(() => {
        ...
        manager.on(handler);
        return () => {
             manager.off && manager.off(handler);
        };
    }, [app]);
    ...
    const attributes = React.useMemo(() => {
             ...
             if (registry) {
                 ...
                 if (edit) {
                     Object.keys(edit).forEach(k => {
                         if (k !== 'type' && k !== 'atomId' && k !== 'position') allKeys.add(k);
                     });
                 }
             }
        });
        ...
        allKeys.forEach(key => {
            ...
            // Re-verify mixed status properly
             if (atomIds.length > 0) {
                 let val0 = app.world.sceneIndex.getAttribute('atom', atomIds[0], key);
                 for(let i=1; i<atomIds.length; i++) {
                     if (app.world.sceneIndex.getAttribute('atom', atomIds[i], key) !== val0) {
                         mixed = true;
                         break;
                     }
                 }
        ...
        <ScrollArea className="h-full">
            <div className="p-4 space-y-6">
                ...
                <div className="flex items-center justify-between pb-2 border-b">
                    <div>
                        <h3 className="text-sm font-semibold leading-none">Selection Inspector</h3>
                        <p className="text-xs text-muted-foreground mt-1.5">
                            {selection.atoms.size} atom{selection.atoms.size !== 1 ? 's' : ''}, {selection.bonds.size} bond{selection.bonds.size !== 1 ? 's' : ''}
                        </p>
                    </div>
                </div>
                ...
                {atomIds.length > 0 && (
                            ...
                            {Object.entries(attributes).map(([key, info]) => {
                                ...
                                return (
                                    <div key={key} className="grid grid-cols-[80px_1fr] items-center gap-3">
                                        <Label className={cn("text-xs font-medium truncate text-muted-foreground", isElement && "text-foreground font-semibold")} title={key}>
                                            {key}
                                        </Label>
                                        ...
                                        {isElement ? (
                                             ...
                                             >
                                                <SelectTrigger className="h-8 text-xs w-full">
                                                    <SelectValue placeholder={info.mixed ? "<Multiple>" : "Select element"} />
                                                </SelectTrigger>
                                                <SelectContent>
                 ...
                 {atomIds.length > 0 && (
                    <div className="pt-4 border-t">
                        <div className="text-xs font-medium text-muted-foreground mb-3 px-1">Add Property</div>
