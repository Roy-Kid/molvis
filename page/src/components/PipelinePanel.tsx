...
export const PipelinePanel: React.FC<PipelinePanelProps> = ({ app }) => {
  ...
  const handleRemoveModifier = (id: string, e: React.MouseEvent) => {
    if (!app) return;
    const pipeline = (app as any).modifierPipeline as ModifierPipeline;
  ...
  const handleToggleModifier = (modifier: Modifier, e: React.MouseEvent) => {
  ...
  return (
    <div className="h-full flex flex-col">
      ...
      {selectedModifier ? (
          ...
          <div className="h-[200px] shrink-0 border-t bg-muted/10 flex items-center justify-center text-sm text-muted-foreground">
